    ...
    def get(self, request, cid, id):
        ...
        if isCourse.exists():
            ...
            if ischapter.exists():
                ...
                chapters = chapter.objects.filter(
                    course=course.objects.get(slug=cid))

                perticularChapter = chapter.objects.filter(
                    Q(course=course.objects.get(slug=cid)) & Q(slug=id))
                ...
                isContent = chapter_content.objects.filter(
                    chapter=chapter.objects.get(slug=id))

                if isContent.exists():
                    for i in isContent:
                        if i.chfiles != 'False':
                            if str(i.chfiles).endswith('mp4') or str(i.chfiles).endswith('mkv'):
                                MediaContentType.append(
                                    {'types': 1, 'id': i.id})
                            else:
                                MediaContentType.append(
                                    {'types': 2, 'id': i.id})
